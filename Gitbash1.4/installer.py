...
class InstallerGUI:
    ...
    def confirm_shortcut(self):
        # Crea il collegamento diretto a launcher.py con icona personalizzata
        self.log_message("Creazione collegamento sul desktop...")
        try:
            # Percorso del file launcher.py nella stessa cartella dell'installer
            base_dir = os.path.dirname(os.path.abspath(__file__))
            target_py = os.path.join(base_dir, "launcher.py")
            if not os.path.exists(target_py):
                raise FileNotFoundError(f"launcher.py non trovato in {base_dir}")
            ...
            # Usa percorso breve (8.3) per pythonw.exe e launcher.py per evitare WinError 206
            def get_short_path(path):
                try:
                    buf = ctypes.create_unicode_buffer(260)
                    get_short = ctypes.windll.kernel32.GetShortPathNameW
                    if get_short(str(path), buf, 260):
                        return buf.value
                except Exception:
                    pass
                return path

            pythonw_short = get_short_path(pythonw)
            target_py_short = get_short_path(target_py)
            create_shortcut((pythonw_short, target_py_short), shortcut_path, icon_path=icon_path, description="Avvia Git Bash Automatico senza console")
    ...
    def ask_shortcut_question(self):
        # Mostra la domanda collegamento nel log con il path selezionato
        self.log_message(f"Percorso: {self.shortcut_folder}")
        self.show_shortcut_buttons()
