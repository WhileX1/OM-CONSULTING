...
//			wp_enqueue_style('mpce-theme');
        ...
        if (isset($id) && !empty($id)) {
            ...
            if (!empty($attachment) && $attachment->post_type === 'attachment') {
                if (wp_attachment_is_image($id)) {
                    ...
                    $error = __("Attachment is not an image", 'motopress-content-editor');
                }
            } else {
                $error = __("Empty attachment or post type not equal 'attachment'", 'motopress-content-editor');
            }
        } else {
            ...
            foreach ($ids as $id) {
                    ...
                    if (!empty($attachment) && $attachment->post_type === 'attachment') {
                        if (wp_attachment_is_image($id)) {
                            ...
                            $imageErrors[] = __("Attachment is not an image", 'motopress-content-editor');
                        }
                    } else {
                        $imageErrors[] = __("Empty attachment or post type not equal 'attachment'", 'motopress-content-editor');
                    }
                }
        } else {
            $error = __("List of images is empty. Select images.", 'motopress-content-editor');
        }
        if (!empty($classes)) $classes = ' ' . $classes;
        if (self::$isNeedFix && empty($mp_style_classes)) {
            if (!empty($custom_class)) $mp_style_classes = $custom_class;
                ...
                foreach ($ids as $id) {
                    ...
                    if (!empty($attachment) && $attachment->post_type === 'attachment') {
                        if (wp_attachment_is_image($id)) {
                            ...
                            $galleryErrors[] = __("Attachment is not an image", 'motopress-content-editor');
                        }
                    } else {
                        $galleryErrors[] = __("Empty attachment or post type not equal 'attachment'", 'motopress-content-editor');
                    }
                }
            } else {
                $error = __("List of images is empty. Select images.", 'motopress-content-editor');
                ...
                if (isset($url['host']) && !empty($url['host']) && isset($url['path']) && !empty($url['path'])) {
                    ...
                    if ($videoSite) {
                        ...
                        if ($videoId) {
                            ...
                            $error = __("Error while getting the video id", 'motopress-content-editor');
                        }
                    } else {
                        $error = __("Incorrect video URL", 'motopress-content-editor');
                    }
                } else {
                    $error = __("Incorrect video URL", 'motopress-content-editor');
                }
            } else {
                $error = __("Error while parsing the video URL", 'motopress-content-editor');
            }
        } else {
            $error = __("Incorrect video URL", 'motopress-content-editor');
        }
    ...
    function mp_gmap($attrs, $content = null, $tag) {
        $result = __("Nothing found. Please enter correct address.", 'motopress-content-editor');
        $address = '';
        $zoom = '';
        extract(shortcode_atts(self::addStyleAtts(self::getAtts($tag)), $attrs));
        ...
        if (!array_key_exists($address, $formattedAddresses)) {
            ...
            if ($data && isset($data->status)) {
                if ($data->status === 'OK') {
                    ...
                    switch ($data->status) {
                        case 'ZERO_RESULTS' : $result = __("Nothing found. Please enter correct address.", 'motopress-content-editor'); break;
    ...
    function mp_wp_audio($attrs, $content = null, $tag) {
        ...
        if ( $mediaIsSet ) {
            ...
            $shortcode = "<p>". __("Audio source is empty.", 'motopress-content-editor') ."</p>";
        }
        if (!empty($classes)) $classes = ' ' . $classes;
        if (self::$isNeedFix && empty($mp_style_classes)) {
            if (!empty($custom_class)) $mp_style_classes = $custom_class;
    ...
    public function mp_table($attrs, $content = null, $tag) {
        ...
        if (!empty($content)) {
            ...
            $result .= __("Table data is empty.", 'motopress-content-editor');
        }
        $result .= '</div>';
                ...
                if (!empty($matches[1])) {
                	...
                	switch ($templateName) {
		                case 'CEPostsGridTemplate1':
		                	$name = __("Featured Image, Title, Content", 'motopress-content-editor');
		                	break;
		                case 'CEPostsGridTemplate2':
		                	$name = __("Title, Featured Image, Content", 'motopress-content-editor');
		                	break;
		                case 'CEPostsGridTemplate3':
		                	$name = __("Featured Image left, Title, Content", 'motopress-content-editor');
		                	break;
	                }
